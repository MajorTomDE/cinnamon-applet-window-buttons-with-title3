// Cinnamon Applet: Window Buttons with Title Reloaded v3
// Release Date: November 10, 2023
// Released from: https://github.com/MajorTomDE/cinnamon-applet-window-buttons-with-title3
// Forked from: https://github.com/mash-00/window-buttons-with-title2

// Original Author: Fatih Mete
// Maintenance: Hans Peyrot
...
const UUID = "window-buttons-with-title3@majortom";
...
WindowButtonApplet.prototype = {
	...
	try {
		...
		this.settings.bind("title-button-action", "titleButtonAction", this.on_settings_changed);
		this.settings.bind("title-middle-button-action", "titleMiddleButtonAction", this.on_settings_changed);
		this.settings.bind("icon-button-action", "iconButtonAction", this.on_settings_changed);
		this.settings.bind("icon-middle-button-action", "iconMiddleButtonAction", this.on_settings_changed);
		this.settings.bind("show-panels", "show_panels", this.on_settings_changed);
		this.settings.bind("text-mode", "textMode", this.on_settings_changed);
		this.settings.bind("only-maximized", "onlyMaximized", this.on_settings_changed);
		this.settings.bind("title-desktop-text", "titleDesktopText", this.on_settings_changed);
		this.settings.bind("title-nowindow-text", "titleNowindowText", this.on_settings_changed);
		this.settings.bind("title-font", "titleFont", this.on_settings_changed);
		this.settings.bind("title-font-style", "titleFontStyle", this.on_settings_changed);
		this.settings.bind("title-font-color", "titleFontColor", this.on_settings_changed);
		this.settings.bind("title-font-size", "titleFontSize", this.on_settings_changed);
		...
		this._session = new GnomeSession.SessionManager();
            ...
            try {
                global.window_manager.connect("size-change", Lang.bind(this, this._sizeChange));
            } catch (e) {
	...
	fontStyle: function() {
		let titleStyle = "";
		if (this.titleFont) {
			titleStyle = titleStyle + 'font-family:"' + this.titleFont + '";';
		}
		if (this.titleFontStyle) {
			titleStyle = titleStyle + "font-style:" + this.titleFontStyle + ";";
		}
		if (this.titleFontColor) {
			titleStyle = titleStyle + "color:" + this.titleFontColor + ";";
		}
		if (this.titleFontSize != 0) {
			titleStyle = titleStyle + "font-size:" + this.titleFontSize + "px;";
		}
		return titleStyle;
	},

	titleButton: function() {
		this.button["title"] = new St.Button({
			name: "titleButton",
		...
		let label = new St.Label({
			text: this.titleNowindowText.toString(),
			style_class: "window-list-item"
		});
	...
	_sizeChange: function(wm, actor, change) {
/*
...
*/
	...
	onlyMaximize: function() {
		...
		if (app && w.get_maximized()) {
			for (let i = 0; i < buttons.length; ++i) {
				this.button[buttons[i]].show(); //On error comment out
				this.button[buttons[i]].opacity = 255;
			}
		} else {
			for (let i = 0; i < buttons.length; ++i) {
				this.button[buttons[i]].hide(); //On error comment out
				if (buttons[i] != "icon") {
...
function main(metadata, orientation, panelHeight, instance_id) {
    return new WindowButtonApplet(orientation, metadata, panelHeight, instance_id);
}
